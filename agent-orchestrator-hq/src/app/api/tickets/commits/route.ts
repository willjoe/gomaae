...
export async function GET(request: Request) {
  try {
    ...
    const ticket = db.prepare('SELECT identifier FROM tickets WHERE id = ?').get(ticketId);
    ...
    const commits = await listBranchCommits(ticketRepoDir(workspaceRoot, ticket.identifier), branch);
