...
export default function ConnectivityModal({ type, isOpen, onClose }: ConnectivityModalProps) {
    ...
    switch (type) {
      case 'repository':
        return {
          ...
          platforms: [
            { id: 'github', name: 'GitHub', description: 'Host source code & enable remote backup.', icon: <Globe size={20} className="text-blue-500 dark:text-blue-400" /> },
            { id: 'gitlab', name: 'GitLab', description: 'Internal or cloud-hosted management.', icon: <Database size={20} className="text-orange-500 dark:text-orange-400" /> },
      ...
      case 'sync':
        return {
          ...
          platforms: [
            { id: 'linear', name: 'Linear', description: 'High-performance team tracking.', icon: <Monitor size={20} className="text-purple-600 dark:text-purple-400" /> },
            { id: 'github-issues', name: 'GitHub Issues', description: 'Sync with GitHub labels & issues.', icon: <Globe size={20} className="text-blue-500 dark:text-blue-400" /> },
      ...
      case 'docs':
        return {
          ...
          platforms: [
            { id: 'notion', name: 'Notion', description: 'Workspace for wiki and projects.', icon: <PenTool size={20} className="text-foreground" /> },
            { id: 'confluence', name: 'Confluence', description: 'Enterprise team collaboration.', icon: <Book size={20} className="text-blue-600 dark:text-blue-500" /> },
            { id: 'google-docs', name: 'Google Docs', description: 'Real-time collaborative editing.', icon: <FileText size={20} className="text-blue-500 dark:text-blue-400" />, status: 'Soon' }
          ]
        };
      case 'ai':
        return {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal Card */}
      <div className="relative w-full max-w-2xl bg-card border border-border rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 transition-colors duration-300">
        {/* Header */}
        <div className="p-6 border-b border-border flex items-center justify-between bg-muted/30 dark:bg-slate-900/50">
          <div className="flex items-center gap-3 text-left">
             <div className={cn("p-2 rounded-xl bg-card border border-border shadow-sm transition-colors", data.color)}>
           ...
           <div className="grid gap-3">
              {data.platforms.map((p) => (
                <div
                  key={p.id}
                  className={cn(
                    "p-5 bg-muted/20 border border-border rounded-2xl flex items-center justify-between transition-all group",
                    p.status !== 'Soon' ? "hover:border-blue-500/30 hover:bg-blue-600/5 cursor-pointer shadow-sm" : "opacity-50 grayscale cursor-not-allowed"
                  )}
                  onClick={() => p.status !== 'Soon' && (window.location.href = '/settings')}
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-card rounded-xl flex items-center justify-center border border-border shadow-inner group-hover:scale-105 transition-transform">
                        {p.icon}
                    </div>
                    <div>
                        <h3 className="font-bold text-foreground opacity-90">{p.name}</h3>
                        <p className="text-[11px] text-muted-foreground">{p.description}</p>
                    </div>
                  </div>
                  {p.status === 'Soon' ? (
                    <span className="text-[9px] font-bold uppercase text-muted-foreground bg-muted px-2 py-1 rounded">Beta</span>
                  ) : (
                    <ChevronRight size={16} className="text-muted-foreground group-hover:text-blue-500 transition-all" />
                  )}
                </div>
              ))}
           </div>
