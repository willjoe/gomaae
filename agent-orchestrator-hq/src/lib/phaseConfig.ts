...
/**
 * Per-lifecycle color codes. Each lifecycle phase owns one accent color:
 *   initiative -> amber (dark yellow), planning -> blue, development -> violet,
 *   testing -> red, release/operation -> emerald.
 * Class strings are written out in full so Tailwind's scanner can see them.
 */
export interface PhaseTheme {
  label: string;
  // Sidebar nav states
  navActive: string;      // active link background + text
  navActiveSub: string;   // active link sub-label text
  navHover: string;       // inactive link hover background + text
  navHoverIcon: string;   // inactive link icon hover color
  // Generic accents (role cards, badges, page identity)
  text: string;           // accent text color
  border: string;         // resting border
  hoverBorder: string;    // hover border
  iconBox: string;        // soft icon chip (bg + text)
  solidIcon: string;      // solid icon chip background
  badge: string;          // small pill (bg + text + border)
  ring: string;           // selected ring + shadow tint
  selectedText: string;   // selected label text
}
...
export function getStatusDotClasses(status: string): string {
  switch (status) {
    ...
    case 'In Progress': return 'bg-blue-500 animate-pulse shadow-[0_0_8px_rgba(59,130,246,0.4)]';
...
/**
 * Agent execution state — an internal orchestration attribute, stored in its
 * own `agent_state` column and kept SEPARATE from the ticket `status` (which
 * stays standard for Jira/Linear/Asana sync). Indicates the "In Queue" /
 * provisioning sub-state while an agent spins up its environment.
 */
