...
export async function POST(request: Request) {
  try {
    ...
    const { name, description, personality_vector, default_model } = await request.json();
    const id = `role-${uuidv4().substring(0, 8)}`;

    db.prepare('INSERT INTO agent_roles (id, name, description, personality_vector, default_model) VALUES (?, ?, ?, ?, ?)')
      .run(id, name, description, personality_vector || null, default_model || null);

    return NextResponse.json({ success: true, role: { id, name, description, personality_vector, default_model } });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  try {
    const { id, name, description, personality_vector, default_model } = await request.json();
    ...
    if (default_model !== undefined) { sets.push('default_model = ?'); vals.push(default_model || null); }
